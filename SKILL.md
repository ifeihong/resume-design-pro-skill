...
7. **Profile Export** (`profile-export.html`) [v1.5]
   - Language-targeted recruitment profile optimization with copy-paste ready content
   - Auto-selects region based on resume language: Chinese (Boss直聘, 51job), English (LinkedIn, Indeed), Japanese (リクナビ, マイナビ), Korean (잡코리아, 사람인)
   - Dynamically generates ALL work experiences (no hardcoded limit)
   - Each platform has character limits, copy buttons, and optimization tips
   - Tab-based UI for platform switching within the same region
...
✅ resume-zhangsan-glassmorphism.html  (Visual — open in browser)
     ...
     ✅ profile-export.html                 (Language-targeted profiles — auto-selects Boss直聘/51job, LinkedIn/Indeed, リクナビ/マイナビ, or 잡코리아/사람인 based on resume language)
...
AI: [解析完成] 我已提取到以下信息：
    ...
    📸 【头像设置】
    请选择头像方式：
    [1] 上传照片 → 提供图片文件路径（支持 JPG/PNG，建议 300×300 以上）
    [2] 使用抽象头像 → 根据姓名首字母自动生成渐变几何头像
    [3] 不使用头像 → 姓名放大展示，更简洁
    
    💡 提示：头像会在视觉简历中展示，ATS 版本自动隐藏

User: 2

AI: ✅ 已选择【抽象头像】，将根据姓名自动生成
...
AI: [Generates resume + Auto-export PDF]
     ...
     ✅ resume-zhangsan.md                  (Backup — edit anytime)
     ✅ cover-letter-zhangsan-tencent.html  (Cover Letter — matching style)
     ✅ profile-export.html                 (Language-targeted profiles — auto-selects platforms based on resume language)
     ✅ job-search-kit.html                 (Job Search Strategy Report)
     ✅ interview-prep.html                 (Interview Preparation Report)
     ✅ resume-diagnosis.html               (Resume Diagnosis — JD matching)
